...
if [ -f "$FICHIER_SAUVE_APRES" ]; then
    # Utiliser -a pour forcer le traitement en mode texte
    FIN_SAUVE_APRES=$(grep -a "Fin sauvegarde physique" "$FICHIER_SAUVE_APRES" | tail -1 | grep -oE '[0-9]{2}:[0-9]{2}:[0-9]{2}')
else
...
bash $HOME/Bulletin_Q/generate_bulletin.sh